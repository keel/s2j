			...
			if (s.getName().equals("if")) {
				...
				if (tag.getLineNum()< ifs.getLineNum()) {
					...
					if (s1.getName().equals("if") && s1.getState() == Sentence.STATE_DOING) {
						//如为while后面必定紧跟if
						ifs = (IfSentence)s1;
						int[] ifR = this.defineIfBlock(firstIfIndex, false);
						int lastIfTagIndex = this.senList.indexOf(((IfSentence)this.senList.get(ifR[1])).getCondTag());
						//其if块最终tag在return之后则为reverseWhile
						if (lastIfTagIndex > this.returnIndex) {
							isReverseWhile = true;
						}
					...
					if(isReverseWhile){
						...
						if (s2.getName().equals("tag")) {
							...
							CondTagSentence outWhileGtag = null;
							...
							for (int j = ori-2; j >= 0; j--) {
								...
								if (sj.getName().equals("condTag")) {
									CondTagSentence gtt = (CondTagSentence)sj;
							...
							int[] ifRE = this.defineIfBlock(ori, false);
							...
							//如果if块最终tag在外部while的condTag之上,即为倒置的while或continue
...
//								}
							...
							//FIXME if块的上一语句为内容,则一定为do while
	 ...
	 * @param isReverse 是否反向,为true时从beginIndex向上查找
	 * @return 数组:[条件块真正结束的index,最后一个if的index]
	 */
	private int[] defineIfBlock(int beginIndex,boolean isReverse){
...
//			}
//		}
		int[] ifRE = this.defineIfBlock(firstIfIndex, false);
...
//		conds = null;
	...
	private void scanWhileStruct(){
		...
		for (int i = 0; i < this.len; i++) {
			Sentence s = this.senList.get(i);
			if (s.getName().equals("condTag") && s.getState() == Sentence.STATE_DOING) {
				CondTagSentence gtTagSen = (CondTagSentence)s;
				...
				//if (sen.getName().equals("if")) {
					String condTag = gtTagSen.getTag();
					//第一个if
...
//					lastCondIndex = this.senList.indexOf(lastCond);
//					conds = null;
					int[] ifRE = this.defineIfBlock(lastCondIndex, false);
					lastCondIndex = ifRE[1];
					lastCond = (IfSentence) this.senList.get(lastCondIndex);
...
//						}
					...
					for (int j = start+1; j < this.len; j++) {
						...
						if (sg.getName().equals("goto")) {
							GotoSentence gsg = (GotoSentence)sg;
							if (gsg.getTarget().equals(condTag)) {
								gotoS = j;
								gs = gsg;
							}
						}
...
//						if (gs.getTarget().equals(condTag)) {
						//确认为while结构
						ifs.setWhile();
						isWhile = true;
//							break;
...
//	}
	...
	private void backReturnSens(){
		...
		for (int i = len-1; i >= 0; i--) {
			...
			if(returnToEnd == 2){
				if (returnIndex == -1 && s.getName().equals("return")) {
		...
		if (returnIndex > -1) {
			...
			while (true) {
				...
				if (s1.getName().equals("return")) {
					...
					}else if(s1.getName().equals("condTag")){
						((GotoSentence)((CondTagSentence)s1).getIfSen()).setReturn(true);
					}
